    ...
    return (
        <SafeAreaContainerView>
            <ScrollView style={{ width: '100%', height: 700, marginBottom: 140 }}>
                ...
                <View style={{...globalStyles.flexColView, marginTop: 30, paddingHorizontal: 12, gap: 30}}>
                    {commentUsers.map((commentUser, index) => (
                        <TouchableOpacity
                            key={index}
                            style={{...globalStyles.flexRowView, justifyContent: 'flex-start', gap: 15}}
                            onPress={() => navigation.navigate('Profile')}
                        >
                            <Image source={commentUser.avatar} style={{ width: 50, height: 50, borderRadius: 50 }} />
                            <Text style={{ color: '#000', fontWeight: '700', fontSize: 16 }}>
                                {commentUser.name}&nbsp;
                                {commentUser.poster ?
                                    <Text style={{ color: 'gray', fontWeight: '400' }}> This is the caption of the post </Text> :
                                    <Text style={{ color: 'gray', fontWeight: '400' }}> comment on post </Text>
                                }
                            </Text>
                        </TouchableOpacity>
                    ))}
            ...
            >
                ...
                >
                    <Text style={{ color: '#000', fontSize: 20, fontWeight: '800' }}> Post </Text>
                </TouchableOpacity>
            </View>
