...
import { View, Image, Text, TouchableOpacity, ScrollView, ToastAndroid, Pressable, StyleSheet } from 'react-native';
...
import ColoredIconButton from '../../components/form/ColoredIconButton';
        ...
        if (selectMultiple) {
            ...
            if (exist) {
                ...
                if (selectedImages.length > 9) {
                    ToastAndroid.show("Hey! Kidding? You have already picked all 10 images.", ToastAndroid.LONG);
                    return;
                } else {
                    temp.push(gallery);
                }
    ...
    return (
        <SafeAreaContainerView>
            <View
                ...
                {/* <FontAwesomeIcon icon="fa-light fa-xmark" style={{color: "#ffffff",}} /> */}
                ...
                <TouchableOpacity
                    onPress={() =>
                        navigation.navigate('Filter', {
                            image: zoomInOut ? croppedImg.current : Image.resolveAssetSource(galleries.find(gallery => gallery.id === selectedId)?.src).uri,
                        })
                    }>
                    <Text
                        style={{
            ...
            <View style={{ ...globalStyles.flexRowView, position: 'relative', overflow:'hidden'}}>
                ...
                {zoomInOut ? 
                    ...
                    image={
                        Image.resolveAssetSource(galleries.find(gallery => gallery.id === selectedId)?.src/*require('../../assets/images/gallery/gallery(16).png')*/)//galleries.find(gallery => gallery.id === selectedId)?.src,
                    }
                ...
                {!zoomInOut ? (selectMultiple === false && galleries.find(gallery => gallery.id === selectedId)?.type !== 'video' && 
                // <Button
                // icon={
                //     <Icon
                ...
                <IconButton
                    ...
                    onPress={OnPressZoom}
                />
                ) : (selectMultiple === false && galleries.find(gallery => gallery.id === selectedId)?.type !== 'video' && 
                // <Button
                // icon={
                //     <Icon
                //     name="unfold-more-vertical"
                ...
                {/* 
                :
                <Video source={galleries.find(gallery => gallery.id === selectedId)?.src} style={{ width: '100%', height: SCREEN_HEIGHT / 2 - 50 }} controls={true} repeat={true} paused={false} resizeMode={'cover'} />
            ...
            <View
                ...
                <View
                    ...
                    {selectMultiple === false ? (
                        ...
                        <Button
                            ...
                            buttonStyle={{borderRadius:30, backgroundColor:'#282c34', paddingVertical:7}}
                        ...
                        <Button
                            ...
                            buttonStyle={{borderRadius:30, backgroundColor:'#1e7fc9', paddingVertical:7}}
                            containerStyle={{paddingVertical:1}}
                ...
                {selectMultiple && <View style={styles.tooltip}>
                    <Text style={{ color: '#FFF', fontSize: 13 }}>Share up to 10 photos and videos in one post.</Text>
                </View>}
                {selectMultiple && <FontAwesomeIcon
                    icon={faSortDesc}
                    style={{ color: '#282c34', position: 'absolute', top: -20, right: 52 }}
                    size={30}
                />}
            </View>
                    ...
                    (
                        <View key={index}>
                            <TouchableOpacity onPress={() => {
                                ...
                                />
                                {/* {gallery.type === 'video' &&
                                    <View style={{ position: 'absolute', bottom: 0, right: 0, backgroundColor: '#333', width: 25 }}>
                                        <Text style={{ color: '#FFF', textAlign: 'center' }}>{gallery.time}s</Text>
                                    </View>
                                } */}
                                {/* {gallery.type === 'video' &&
                                    <View style={{ position: 'absolute', top: 0, left: 0, backgroundColor: '#333', width: 25, alignItems: 'center' }}>
                                        <FontAwesomeIcon size={20} icon={faVideo} style={{ color: '#FFF' }} /> 
                                    </View>
                                } */}
                                {selectMultiple &&
                                    <View
                                        style={{
                                            backgroundColor: getIndex(gallery) === -1 ? 'white' : '#1e7fc9',
                                            borderRadius: 15,
                                            alignItems: 'center',
                                            display: 'flex',
                                        ...
                                        <Text style={{ color: '#fff' }}>{getIndex(gallery) === -1 ? '' : getIndex(gallery) + 1}</Text>
                                    </View>
                                }
                            </TouchableOpacity>
                        </View>
